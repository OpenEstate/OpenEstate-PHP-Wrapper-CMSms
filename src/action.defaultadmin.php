...
if ($environmentIsValid) {
  ...
  $this->smarty->assign('input_index_lang', $this->CreateInputSelectList(
          $id, 'index_lang', $langs, array($defaultLang), 0, 'id="tag_index_lang" onchange="build_tag();"', false));
  $this->smarty->assign('input_expose_lang', $this->CreateInputSelectList(
          $id, 'expose_lang', $langs, array($defaultLang), 0, 'id="tag_expose_lang" onchange="build_tag();"', false));
  ...
  $this->smarty->assign('input_index_view', $this->CreateInputSelectList(
          $id, 'index_view', $views, array('index'), 0, 'id="tag_index_view" onchange="build_tag();"', false));
  ...
  $this->smarty->assign('input_index_mode', $this->CreateInputSelectList(
          $id, 'index_mode', $modes, array('entry'), 0, 'id="tag_index_mode" onchange="build_tag();"', false));
  ...
  $this->smarty->assign('input_index_order_by', $this->CreateInputSelectList(
          $id, 'index_order_by', $orders, array($defaultOrder), 0, 'id="tag_index_order_by" onchange="build_tag();"', false));
  ...
  $this->smarty->assign('input_index_order_dir', $this->CreateInputSelectList(
          $id, 'index_order_dir', $dirs, array('asc'), 0, 'id="tag_index_order_dir" onchange="build_tag();"', false));
  ...
  if (count($filters) > 0) {
    $this->smarty->assign('input_index_filters', implode('<br/>', $filters));
  }
  if (count($filterIds) > 0) {
    $this->smarty->assign('filter_ids', implode(', ', $filterIds));
  }
  ...
  $this->smarty->assign('input_expose_view', $this->CreateInputSelectList(
          $id, 'expose_view', $views, array('details'), 0, 'id="tag_expose_view" onchange="build_tag();"', false));

  // expose, id
  $this->smarty->assign('input_expose_id', $this->CreateInputText(
          $id, 'expose_id', '', 50, 255, 'onchange="build_tag();" onkeydown="build_tag();" style="width:25em;"'));
}
else {
...
$smarty->assign('input_wrap_url', $this->CreateInputTextWithLabel(
        $id, 'wrap_url', $this->GetPreference('wrap_url', ''), 50, 255, 'style="width:75%;"', $this->Lang('title_wrap_url_help') . '<br/>'));
